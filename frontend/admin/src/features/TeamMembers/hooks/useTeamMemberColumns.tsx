...
type UseTeamMemberColumnsProps = {
  handleEdit: (teamMember: TeamMember) => void;
  handleDelete: (teamMember: TeamMember) => void;
  handleToggleActive: (teamMember: TeamMember) => void;
};
...
export function useTeamMemberColumns({
  ...
  handleDelete,
  handleToggleActive
}: UseTeamMemberColumnsProps): ColumnDef<TeamMember>[] {
  ...
  return useMemo(() => [
    ...
    },
    {
      ...
      header: ({ column }) => <TableHeaderSort column={column} title={t("Occupation")} />,
      enableSorting: true,
      cell: ({ row }) => (
        <div className="flex flex-col space-y-1">
          {SUPPORTED_LANGS.map(lang => (
            <div key={lang}>
              {lang.toUpperCase()}: <span>{row.original.occupation?.[lang]}</span>
            </div>
          ))}
        </div>
      ),
    },
    {
      ...
      enableSorting: true,
      cell: ({ row }) => <div>{row.original.arrangement}</div>,
    },
    {
