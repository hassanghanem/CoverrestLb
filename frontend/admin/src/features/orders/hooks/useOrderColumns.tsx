...
import { fetchOrderReceiptHtml } from "@/lib/services/Orders-services";
    ...
    () => [
...

      {
        ...
        cell: ({ row }) => {
          ...
          if (!status) return <Badge className="bg-gray-400 text-white">{t("Unknown")}</Badge>;
          return (
...

      {
        ...
        header: ({ column }) => <TableHeaderSort column={column} title={t("Subtotal")} />,
        enableSorting: true,
        cell: ({ row }) =>
          formatPrice(
            (row.original.subtotal ?? 0) * (selectedCurrency?.exchange_rate ?? 1),
...

      {
        ...
        header: ({ column }) => <TableHeaderSort column={column} title={t("Delivery Fee")} />,
        enableSorting: true,
        cell: ({ row }) =>
          formatPrice(
            (row.original.delivery_amount ?? 0) * (selectedCurrency?.exchange_rate ?? 1),
            selectedCurrency?.code,
            i18n.language
          ),
      },

      {
        ...
        header: ({ column }) => <TableHeaderSort column={column} title={t("Coupon")} />,
        enableSorting: true,
        cell: ({ row }) => {
...

      {
        ...
        header: ({ column }) => <TableHeaderSort column={column} title={t("Total")} />,
        enableSorting: true,
        cell: ({ row }) =>
          formatPrice(
            parseFloat(row.original.grand_total ?? "0") * (selectedCurrency?.exchange_rate ?? 1),
            selectedCurrency?.code,
            i18n.language
          ),
      },

      {
        ...
        header: ({ column }) => <TableHeaderSort column={column} title={t("Payment Method")} />,
        enableSorting: true,
...

      {
        ...
        header: ({ column }) => <TableHeaderSort column={column} title={t("Payment Status")} />,
        enableSorting: true,
        cell: ({ row }) => {
          ...
          const statusInfo = statusMap[row.original.payment_status] || {
            label: t("N/A"),
            color: "bg-gray-400",
          };
        ...
        cell: ({ row }) => (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => navigate(`/orders/${row.original.id}/view`)}>
                {t("View")}
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={async () => {
                  const html = await fetchOrderReceiptHtml(row.original.id);
                  if (!html) return;
