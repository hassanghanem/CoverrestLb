...
export const LocationPicker: React.FC<LocationPickerProps> = ({ value, onChange, disabled = false }) => {
  ...
  const hasValidLocation = useMemo(() => !!value?.lat && !!value?.lon, [value?.lat, value?.lon]);
    ...
    try {
      ...
      switch (error.code) {
        ...
        case error.POSITION_UNAVAILABLE:
          setError(t("Location information unavailable"));
          break;
        case error.TIMEOUT:
          setError(t("Location request timed out"));
          break;
        default:
          setError(t("Unable to retrieve location"));
      }
    } finally {
      setIsLoading(false);
    }
