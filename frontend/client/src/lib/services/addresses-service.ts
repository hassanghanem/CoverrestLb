...
import { GetAddressesResponse, GetAddressResponse } from "@/types/response.interfaces";
...
export const getAddressById = async (id: number): Promise<GetAddressResponse> => {
  try {
...
export const getDefaultAddress = async (): Promise<GetAddressResponse> => {
  try {
    const response = await axiosInstance.get(API_ENDPOINTS.ADDRESSES.DEFAULT);
    return response.data;
  } catch (error: any) {
    return { result: false, message: error?.response?.data?.message };
  }
